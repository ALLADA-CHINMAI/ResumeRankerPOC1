...
container_name_resume = os.getenv("CONTAINER_NAME_RESUME", )
...
index_client = SearchIndexerClient(endpoint=search_service_endpoint, credential=AzureKeyCredential(""))
...
## index_client.run_indexer("resumes-vectorised-indexer-indexer")
#Fetch--------------------------------------------------------------------------------------------
# Get the BlobClient
...
st.title('ResumeRanking POC!')
st.write('this is the preliminary part of ATS')
...
def GetResults():
    ...
    for result in results:
        ...
        scoresList.append([result['title'],result['@search.score'], aiScore])
...
st.title('Upload File to Resume Storage base')
st.write('once file is uploaded, it will be added to scored according to the Job description given, new list will be created accordingly ');
...
if uploaded_file is not None:
    ...
    blob_client = blob_service_client.get_blob_client(container=container_name_resume, blob=uploaded_file.name)
    blob_client.upload_blob(uploaded_file, overwrite=True)
    
    st.success(f"File '{uploaded_file.name}' uploaded successfully to Azure Blob Storage.")
    index_client.run_indexer("rag-1751366501876-indexer")
    st.success(f"File '{uploaded_file.name}' uploaded Vectorised.")
    data = GetResults()
    st.success(f"File '{uploaded_file.name}' is added to list.")
...
data = [["Alice", 30, "New York"], ["Bob", 25, "San Francisco"], ["Charlie", 35, "Chicago"]]

data = GetResults()
# Create a DataFrame
df = pd.DataFrame(data, columns=["Name", "Final Score"])

# Display the DataFrame as a table in Streamlit
st.table(pd.DataFrame(data, columns=["Name", "Final Score"]))
